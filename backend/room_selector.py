...
TOOL_DEFINITIONS: List[ChatCompletionToolParam] = [
    {
        ...
        "function": {
            ...
            "description": "Return available meeting rooms filtered by capacity, equipment, accessibility, distance, and booked time slots.",
            "parameters": {
                "type": "object",
                "properties": {
                    ...
                    "attendees": {"type": "integer", "description": "Number of attendees (capacity required)"},
                    "needs": {"type": "array", "items": {"type": "string"}, "description": "Required equipment list (all must be present)"},
                    "accessible_required": {"type": "boolean", "description": "Whether accessible room is required"},
                },
                "required": ["date", "time", "duration_min", "attendees", "needs", "accessible_required"],
            },
        },
    },
]
...
def prompt_template_baseline() -> PromptTemplate:
    return PromptTemplate(
        template=(
            "You are a scheduling assistant. Given the requirements, return strictly in this format:\n"
            "final_choice: <ROOM_ID or No Room>\n"
            "reason: <one line stating the decisive criteria>\n"
            "If no room fits, return:\n"
            "final_choice: No Room\n"
            "reason: <reason>\n"
            "\n"
            "Example:\n"
            "final_choice: Nova\n"
            "reason: Only room with enough capacity and all required equipment.\n"
            "\n"
            "Requirements: Find a room on {date} at {time} for {duration_min} minutes, {attendees} attendees. Needs: {needs}. Accessible required: {accessible_required}"
        ),
        engine="f-string",
    )
...
def room_selection_grader(client: OpenAI, final_message: Optional[str], expected_choice: str) -> float:
    judge_prompt = (
        f"You are a helpful grader for a room selection task.\n"
        f"Task output:\n{final_message}\n\n"
        f"Task expected answer:\n{expected_choice}\n\n"
        f"Score the match on a 0-1 scale. Give partial credit for close answers, correct room ID, or correct reasoning, even if the format is not exact.\n"
        f"If the answer is correct but the format is slightly off, give at least 0.7. If the room ID is correct but the reason is missing, give at least 0.8. If the answer is wrong, give 0.0.\n"
        f"Be helpful and not overly strict."
    )
    ...
    # Mock token cost: use word count of final_message
    mock_token_cost = len(final_message.split()) if final_message else 0
    lambda_penalty = 0.01  # Penalty per token (adjust as needed)
    reward = judge_result_parsed.score - lambda_penalty * mock_token_cost

    console.print(f"[bold yellow]=== Judge Score ===[/bold yellow]")
    console.print(judge_result_parsed.score)
    console.print(f"[bold yellow]=== Mock Token Cost ===[/bold yellow]")
    console.print(mock_token_cost)
    console.print(f"[bold yellow]=== Final Reward (Score - Cost Penalty) ===[/bold yellow]")
...
def room_selector(task: RoomSelectionTask, prompt_template: PromptTemplate) -> float:
    ...
    # Use the same default as capital_agent.py for compatibility
...
def get_rooms_and_availability(
    ...
    for r in ROOMS:
        ...
        # Check capacity
        enough_capacity = r["capacity"] >= attendees
        # Check equipment
        has_equipment = all(need in r["equipment"] for need in needs)
        # Check accessibility
        accessibility_ok = (not accessible_required) or r["accessible"]
        # Only include if all constraints are satisfied and free
        if free and enough_capacity and has_equipment and accessibility_ok:
            item: RoomStatus = {
                **r,
                "free": free,
            }
